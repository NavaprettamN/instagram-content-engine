...
- **No more silent video reels (2026-07-13)** — root-caused the earlier soundless post: many v.redd.it clips have no audio stream, and `-map 0:a?` shipped them silent. Fix: `MemeAgent.has_audio()` (ffprobe) + `add_music_bed()` (mux looping CC track, `-c:v copy`); `build_video` now detects a silent source and adds a Jamendo bed like image reels. Verified end-to-end: the exact clip that came out silent (`g8igy0q82wch1`) now uploads with audio + CC credit in caption.

### ⏳ Open / offered (not started)
- **CI workflow (ruff + pyright)** — no CI validates code today (Actions only runs the content pipeline). Would auto-catch the code-review's remaining 🟡/🟢 items (deprecated `datetime.utcnow()`, unclosed config.yaml handles, dead `build_linkbio` affiliate/lead_magnet refs). Offered 2026-07-13; awaiting go-ahead.
